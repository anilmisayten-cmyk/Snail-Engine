...
class Init extends FlxState
{
	override public function create():Void
	{
		...
		// PreSplash plays the intro animation before the main Splash screen.
		// If skipSplash is on (debug) or the user turned off the splash toggle,
		// skip both and go straight to the initial state.
		if (Main.startMeta.skipSplash || !ClientPrefs.toggleSplashScreen)
			FlxG.switchState(() -> Type.createInstance(Main.startMeta.initialState, []));
		else
			FlxG.switchState(() -> new PreSplash());
	}
}
