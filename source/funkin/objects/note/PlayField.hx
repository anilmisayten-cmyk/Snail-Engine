	...
	**/
	...
	public function noteHit(note:Note, field:PlayField):Void
	{
		...
		if (strum != null)
		{
			...
			if (note.isSustainEnd && field.playerControls && !field.autoPlayed)
			{
				strum.isHoldingSustain = false;
				strum.playAnim('pressed', true);
			}
			else if (note.isSustainNote && strum.animation.curAnim != null && strum.animation.curAnim.name == 'confirm')
			{
				strum.isHoldingSustain = true;
				// Keep playing the confirm animation without restarting it
			}
			else
			{
				strum.isHoldingSustain = note.isSustainNote;
	...
	{
		if (field.playerControls) FlxG.sound.play(Paths.soundRandom('missnote', 1, 3), FlxG.random.float(0.1, 0.2));
