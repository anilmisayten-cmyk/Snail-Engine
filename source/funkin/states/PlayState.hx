...
class PlayState extends MusicBeatState
{
	...
	**/
	...
	public var vSliceScoreAccum:Float = 0;
	...
	**/
	...
	public var infoCard:funkin.game.SongInfoCard;
	...
	override public function create():Void
	{
		...
		infoCard = new funkin.game.SongInfoCard();
		add(infoCard);
		infoCard.cameras = [camHUD];
	...
	function startSong():Void
	{
		...
		if (infoCard != null) infoCard.animate();
	}
	...
	override public function update(elapsed:Float):Void
	{
		...
		var holdingSustain:Bool = false;
		if (playFields != null) {
			for (field in playFields.members)
			{
				if (field != null && field.playerControls && !field.autoPlayed)
				{
					for (strum in field.members)
					{
						if (strum != null && strum.isHoldingSustain)
						{
							holdingSustain = true;
							break;
						}
				...
				if (holdingSustain) break;
			}
		}

		if (holdingSustain && defaultScoreAddition)
		{
			vSliceScoreAccum += elapsed * 400; // 400 score per second while holding
			var added:Int = 0;
			while (vSliceScoreAccum >= 1)
			{
				songScore += 1;
				vSliceScoreAccum -= 1;
				added++;
			...
			if (added > 0) {
				callHUDFunc(hud -> hud.onUpdateScore(songScore, funkin.utils.MathUtil.floorDecimal(ratingPercent * 100, 2), songMisses, false));
			}
		}

		// 6 tuşuna basınca Botplay'i aç/kapat
