...
class ChartEditorState extends haxe.ui.backend.flixel.UIState
{
	...
	**/
	...
	var draggingNote:Array<Dynamic> = null;
	...
	override function create()
	{
		...
		mouseClickSounds = true;
		
		editorMusic = new flixel.sound.FlxSound().loadEmbedded(Paths.music('musicEditors'), true);
		editorMusic.volume = 0.5;
		editorMusic.play();
		flixel.FlxG.sound.list.add(editorMusic);
	...
	var editorMusic:flixel.sound.FlxSound;
	...
	public function mouseInput(elapsed:Float):Void
	{
		...
		if (draggingNote != null && FlxG.mouse.pressed)
		{
			var yToUse:Float = FlxG.mouse.y;
			if (!FlxG.keys.pressed.SHIFT)
			{
				var gridmult = GRID_SIZE / (quantization / 16);
				yToUse = Math.floor(yToUse / gridmult) * gridmult;
			}
			var newStrum = getStrumTime(yToUse * (getSectionBeats() / 4), false) + sectionStartTime();
			var diff = newStrum - draggingNote[0];
			diff = Math.max(0, diff);
			
			if (diff != draggingNote[2])
			{
				draggingNote[2] = Math.round(diff); // ensure integer sustain length
				updateNoteUI();
				updateGrid();
			}
		}
	...
	public function toggleMusic(play:Bool):Void
	{
		...
		{
			if (editorMusic != null && editorMusic.playing) editorMusic.pause();
		...
		else if (!play && audio.playing)
		{
			...
			audio.pause();
			if (editorMusic != null && !editorMusic.playing) editorMusic.resume();
	...
	function setupSusNote(note:EditorNote, beats:Float):flixel.group.FlxSpriteGroup
	{
		...
		var sprWidth:Float = GRID_SIZE * 0.35;

		var holdBody:EditorNote = new EditorNote(note.strumTime, note.noteData, null, true, true, note.player);
		holdBody.isSustainEnd = false;
		if (note.noteType != null && note.noteType.length > 0) holdBody.noteType = note.noteType;
		holdBody.skipScale = true;
		holdBody.reloadNote();
		holdBody.playAnim(holdBody.getDefaultAnim(), true);
		holdBody.setGraphicSize(Std.int(sprWidth), Std.int(height));
		holdBody.updateHitbox();
		holdBody.x = note.x + (GRID_SIZE / 2) - (holdBody.width / 2);
		holdBody.y = note.y + GRID_SIZE / 2;
		
		var holdEnd:EditorNote = new EditorNote(note.strumTime + note.sustainLength, note.noteData, null, true, true, note.player);
		holdEnd.isSustainEnd = true;
		if (note.noteType != null && note.noteType.length > 0) holdEnd.noteType = note.noteType;
		holdEnd.skipScale = true;
		holdEnd.reloadNote();
		holdEnd.playAnim(holdEnd.getDefaultAnim(), true);
		holdEnd.setGraphicSize(Std.int(sprWidth));
		holdEnd.updateHitbox();
		holdEnd.x = note.x + (GRID_SIZE / 2) - (holdEnd.width / 2);
	...
	{
		funkin.utils.FileUtil.browseForFile(
			{
				typeFilter: [new openfl.net.FileFilter('JSON', 'json')],
				title: 'Load Chart'
			},
			function(path:String) {
				try {
					...
					if (rawJson != null)
					{
						var loadedChart:Dynamic = haxe.Json.parse(rawJson);
						if (loadedChart.song != null)
						{
							song = loadedChart.song;
							if (song.song == null) song.song = 'unknown'; // ensure backwards compatibility
							...
							updateGrid();
							reloadGridLayer();
							ui.bind(); // update ui fields
							updateHeads();
							// Ensure audio is paused after loading to avoid test playback
							toggleMusic(false);
							audio.pause();
							FlxG.log.notice("Successfully loaded chart.");
						}
					}
				} catch (e:Dynamic) {
					FlxG.log.error("Failed to load chart: " + e);
				}
			}
		);
	}
		...
		funkin.utils.FileUtil.browseForFile(
			...
			function(path:String) {
				try {
					...
					if (rawJson != null)
					{
						var loadedEvents:Dynamic = haxe.Json.parse(rawJson);
						if (loadedEvents.song != null && loadedEvents.song.events != null)
						{
							song.events = loadedEvents.song.events;
							updateGrid();
							FlxG.log.notice("Successfully loaded events.");
						}
					}
				} catch (e:Dynamic) {
					FlxG.log.error("Failed to load events: " + e);
				}
			}
		);
	}
